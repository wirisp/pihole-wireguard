...
PresharedKey = $psk
AllowedIPs = 10.2.53.$octet/32$(grep -q 'fc10:253::1' /etc/wireguard/wg0.conf && echo ", fc10:253::$octet/128")
# END_PEER $client
EOF
...
[Interface]
Address = 10.2.53.1/24$([[ -n "$ip6" ]] && echo ", fc10:253::1/64")
...
EOF
	...
	if systemctl is-active --quiet firewalld.service; then
		...
		firewall-cmd -q --zone=trusted --add-source=10.2.53.0/24
		firewall-cmd -q --permanent --add-port="$port"/udp
		firewall-cmd -q --permanent --zone=trusted --add-source=10.2.53.0/24
		# Set NAT for the VPN subnet
		firewall-cmd -q --direct --add-rule ipv4 nat POSTROUTING 0 -s 10.2.53.0/24 ! -d 10.2.53.0/24 -j SNAT --to "$ip"
		firewall-cmd -q --permanent --direct --add-rule ipv4 nat POSTROUTING 0 -s 10.2.53.0/24 ! -d 10.2.53.0/24 -j SNAT --to "$ip"
		if [[ -n "$ip6" ]]; then
			firewall-cmd -q --zone=trusted --add-source=fc10:253::/64
			firewall-cmd -q --permanent --zone=trusted --add-source=fc10:253::/64
			firewall-cmd -q --direct --add-rule ipv6 nat POSTROUTING 0 -s fc10:253::/64 ! -d fc10:253::/64 -j SNAT --to "$ip6"
			firewall-cmd -q --permanent --direct --add-rule ipv6 nat POSTROUTING 0 -s fc10:253::/64 ! -d fc10:253::/64 -j SNAT --to "$ip6"
		fi
	else
...
[Service]
Type=oneshot
ExecStart=$iptables_path -t nat -A POSTROUTING -s 10.2.53.0/24 ! -d 10.2.53.0/24 -j SNAT --to $ip
ExecStart=$iptables_path -I INPUT -p udp --dport $port -j ACCEPT
ExecStart=$iptables_path -I FORWARD -s 10.2.53.0/24 -j ACCEPT
ExecStart=$iptables_path -I FORWARD -m state --state RELATED,ESTABLISHED -j ACCEPT
ExecStop=$iptables_path -t nat -D POSTROUTING -s 10.2.53.0/24 ! -d 10.2.53.0/24 -j SNAT --to $ip
ExecStop=$iptables_path -D INPUT -p udp --dport $port -j ACCEPT
ExecStop=$iptables_path -D FORWARD -s 10.2.53.0/24 -j ACCEPT
ExecStop=$iptables_path -D FORWARD -m state --state RELATED,ESTABLISHED -j ACCEPT" > /etc/systemd/system/wg-iptables.service
		if [[ -n "$ip6" ]]; then
			echo "ExecStart=$ip6tables_path -t nat -A POSTROUTING -s fc10:253::/64 ! -d fc10:253::/64 -j SNAT --to $ip6
ExecStart=$ip6tables_path -I FORWARD -s fc10:253::/64 -j ACCEPT
ExecStart=$ip6tables_path -I FORWARD -m state --state RELATED,ESTABLISHED -j ACCEPT
ExecStop=$ip6tables_path -t nat -D POSTROUTING -s fc10:253::/64 ! -d fc10:253::/64 -j SNAT --to $ip6
ExecStop=$ip6tables_path -D FORWARD -s fc10:253::/64 -j ACCEPT
	...
	case "$option" in
		...
		4)
			...
			if [[ "$remove" =~ ^[yY]$ ]]; then
				...
				if systemctl is-active --quiet firewalld.service; then
					ip=$(firewall-cmd --direct --get-rules ipv4 nat POSTROUTING | grep '\-s 10.2.53.0/24 '"'"'!'"'"' -d 10.2.53.0/24' | grep -oE '[^ ]+$')
					# Using both permanent and not permanent rules to avoid a firewalld reload.
					firewall-cmd -q --remove-port="$port"/udp
					firewall-cmd -q --zone=trusted --remove-source=10.2.53.0/24
					firewall-cmd -q --permanent --remove-port="$port"/udp
					firewall-cmd -q --permanent --zone=trusted --remove-source=10.2.53.0/24
					firewall-cmd -q --direct --remove-rule ipv4 nat POSTROUTING 0 -s 10.2.53.0/24 ! -d 10.2.53.0/24 -j SNAT --to "$ip"
					firewall-cmd -q --permanent --direct --remove-rule ipv4 nat POSTROUTING 0 -s 10.2.53.0/24 ! -d 10.2.53.0/24 -j SNAT --to "$ip"
					if grep -qs 'fc10:253::1/64' /etc/wireguard/wg0.conf; then
						ip6=$(firewall-cmd --direct --get-rules ipv6 nat POSTROUTING | grep '\-s fc10:253::/64 '"'"'!'"'"' -d fc10:253::/64' | grep -oE '[^ ]+$')
						firewall-cmd -q --zone=trusted --remove-source=fc10:253::/64
						firewall-cmd -q --permanent --zone=trusted --remove-source=fc10:253::/64
						firewall-cmd -q --direct --remove-rule ipv6 nat POSTROUTING 0 -s fc10:253::/64 ! -d fc10:253::/64 -j SNAT --to "$ip6"
						firewall-cmd -q --permanent --direct --remove-rule ipv6 nat POSTROUTING 0 -s fc10:253::/64 ! -d fc10:253::/64 -j SNAT --to "$ip6"
					fi
				else
